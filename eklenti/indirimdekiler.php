...
tema_eklenti_ac("İndirimdekiler");
$i=0;
$a=0;
$sql=mysql_query("SELECT * FROM urunler WHERE indirim=1 ORDER BY no DESC");
...
{
if($i==5)break;
echo "<li onmouseover=\"urun_goster('indirim_$s->no',1)\" onmouseout=\"urun_goster('indirim_$s->no',0)\">";
echo "<a href=urun.php?no=$s->no>
<img class=resim src=resimler/$s->no/$s->resim>
<span class=baslik>$s->ad</span>
</a>";

echo "<div id=indirim_$s->no class=eklenti-liste-urun>";
